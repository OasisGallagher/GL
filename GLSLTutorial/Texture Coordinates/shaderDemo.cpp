...
void renderScene(void) {
	...
	float lightPos[4] = { 0, 4.f, 0, 1.f };
	float res[4];
	vsml->multMatrixPoint(VSMathLib::VIEW, lightPos, res);
	// transform position to camera space, and send to GLSL.
	shader.setBlockUniform("Lights", "l_pos", res);

	float lightDir[4] = { 0, -1.f, 0, 0 };
	...
	float spotCutOff = cos(15 * deg2Rad);

	shader.setBlockUniform("Lights", "l_spotCutOff", &spotCutOff);

	float diffuse[] = { 0.f, 0.8f, 0.f, 1.f };
...
GLuint setupShaders() {
	...
	shader.loadShader(VSShaderLib::FRAGMENT_SHADER, "shaders/color3.frag");

	// set semantics for the shader variables
	// 与在shader中加入layout(location = 0) outputF一样, 且二者同时存在时,
	// shader中的优先级更高.
